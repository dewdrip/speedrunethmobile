...
describe('Snowman☃️', () => {
  ...
  describe('addAccessory()', () => {
    it('adds accessory', async () => {
      ...
      expect(
        (await snowman.getAccessories()).some(
          accessory => accessory._address === hat.target
        )
      ).to.be.true;
    ...
    it('reverts if caller is not owner', async () => {
      await expect(snowman.addAccessory(hat.target, 1)).to.be.revertedWith(
    ...
    it('reverts if accessory already exists', async () => {
      await snowman.connect(owner).addAccessory(hat.target, 1);
      await expect(
        snowman.connect(owner).addAccessory(hat.target, 1)
      ).to.revertedWithCustomError(snowman, 'Snowman__AcccessoryAlreadyExists');
    });
  });
  ...
  describe('addAccessories()', () => {
    it('adds accessories', async () => {
      ...
      expect(
        (await snowman.getAccessories()).some(accessory =>
          accessories.includes(accessory._address)
        )
      ).to.be.true;
    ...
    it('emits an event', async () => {
      await expect(
        snowman.connect(owner).addAccessories(accessories, [1, 0, 0])
      )
    ...
    it('reverts if caller is not owner', async () => {
      await expect(
        snowman.addAccessories(accessories, [1, 0, 0])
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });
    it('reverts if accessories length does not match positions length', async () => {
      await expect(
        snowman.connect(owner).addAccessories(accessories, [1, 0])
  ...
  describe('removeAccessory()', () => {
    ...
    it('reverts if caller is not snowman owner', async () => {
      await expect(
        snowman.connect(owner).removeAccessory(hat.target, 1)
      ).to.be.revertedWithCustomError(snowman, 'Snowman__NotOwner');
    });
    it('reverts if accessory is not available', async () => {
      await expect(
        snowman.removeAccessory(belt.target, 1)
      ).to.be.revertedWithCustomError(snowman, 'Snowman__UnavailableAccessory');
    });
    it('reverts if accessory is not worn', async () => {
      await expect(
        snowman.removeAccessory(scarf.target, 1)
      ).to.be.revertedWithCustomError(snowman, 'Snowman__AccessoryNotWorn');
    });
  ...
  describe('removeAllAccessories()', () => {
    ...
    it("removes all accessories from user's snowman and emits an event", async () => {
      ...
      await expect(snowman.removeAllAccessories(1)).to.emit(
        snowman,
        'AccessoriesRemoved'
      );
    ...
    it('reverts if caller is not snowman owner', async () => {
      await expect(
        snowman.connect(owner).removeAllAccessories(1)
      ).to.be.revertedWithCustomError(snowman, 'Snowman__NotOwner');
    });
  });
  ...
  describe('onERC721Received', () => {
    ...
    it('reverts if caller is not snowman owner', async () => {
      ...
      await expect(
        hat['safeTransferFrom(address,address,uint256,bytes)'](
          valentine.address,
          snowman.target,
          1,
          encodedSnowmanId
